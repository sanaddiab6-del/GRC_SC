...
class RequirementMappingSetReadSerializer(BaseModelSerializer):
    ...
    def _get_framework_info(self, urn):
        """Look up framework info from the pre-fetched context dict (no DB hit)."""
        framework_dict = self.context.get("framework_dict")
        if framework_dict is not None:
            return framework_dict.get(urn, {"str": urn, "urn": urn})
        # Fallback for contexts without pre-fetch
        ...
        return {
            "str": framework.get("name") or framework.get("urn"),
    ...
    def get_source_framework(self, obj):
        ...
        return self._get_framework_info(mapping_set.get("source_framework_urn", ""))

    def get_target_framework(self, obj):
        mapping_set = obj.content.get(
            "requirement_mapping_sets", [obj.content.get("requirement_mapping_set", {})]
        )[0]
        return self._get_framework_info(mapping_set.get("target_framework_urn", ""))
    ...
    def get_frameworks_available(self, obj):
        ...
        framework_dict = self.context.get("framework_dict")
        if framework_dict is not None:
            return source_urn in framework_dict and target_urn in framework_dict
        source = self._get_framework_info(source_urn)
        target = self._get_framework_info(target_urn)
