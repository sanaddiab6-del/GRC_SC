...
class RequirementMappingSetViewSet(BaseModelViewSet):
    ...
    def get_serializer_context(self):
        context = super().get_serializer_context()
        # Pre-fetch all framework libraries once to avoid N+1 queries in the
        # serializer's get_source_framework / get_target_framework methods.
        framework_dict = {}
        for lib in StoredLibrary.objects.filter(
            ...
            framework = lib.content.get("framework", {})
            urn = framework.get("urn")
            if urn:
                framework_dict[urn] = {
                    "str": framework.get("name") or urn,
                    "urn": urn,
                }
        context["framework_dict"] = framework_dict
        return context
