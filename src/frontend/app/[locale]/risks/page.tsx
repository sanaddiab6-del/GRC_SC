          ...
          <div className="flex items-center justify-end">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onSelect={() => setVisibleColumns((prev) => ({ ...prev, likelihood: !prev.likelihood }))}>
                  {visibleColumns.likelihood ? t('hide') : t('show')} {t('likelihood')}
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={() => setVisibleColumns((prev) => ({ ...prev, impact: !prev.impact }))}>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {visibleColumns.likelihood && <TableHead>{t('likelihood')}</TableHead>}
              ...
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((risk) => (
                  <TableRow key={risk.risk_id}>
                    ...
                    {visibleColumns.type && <TableCell>{risk.category || '--'}</TableCell>}
                    {visibleColumns.likelihood && <TableCell>{risk.likelihood ?? '--'}</TableCell>}
                    {visibleColumns.impact && <TableCell>{risk.impact ?? '--'}</TableCell>}
                    <TableCell>{risk.residual_risk_score ?? risk.inherent_risk_score ?? '--'}</TableCell>
                    <TableCell>
                      <Badge
                        variant={severityVariant[risk.residual_risk_level || risk.inherent_risk_level || ''] || 'muted'}
                      >
                        {severityLabel(risk.residual_risk_level || risk.inherent_risk_level || '')}
                      </Badge>
                    </TableCell>
                    <TableCell>
                    ...
                    {visibleColumns.owner && <TableCell>{risk.risk_owner || '--'}</TableCell>}
