...
def _make_incident(user_id=None, override: dict | None = None):
    ...
    m.status = IncidentStatus.OPEN
    m.severity = IncidentSeverity.HIGH
    m.category = IncidentCategory.SECURITY_BREACH
    m.reported_by_id = str(user_id or uuid4())
    m.control_id = None
    m.contained_at = None
    m.resolved_at = None
    m.closed_at = None
...
async def test_list_playbooks(app_with_overrides):
    ...
    from incident.models import IncidentPlaybook

    playbook = MagicMock(spec=IncidentPlaybook)
    playbook.id = str(uuid4())
    playbook.name_en = "Playbook 1"
    playbook.name_ar = "دليل 1"
    playbook.category = "SECURITY_BREACH"
    playbook.steps_en = ["Step 1"]
    playbook.steps_ar = ["خطوة 1"]
